...
#[test]
fn test_snapshot_prune_reports_invalid_source_metadata() {
    let context = TestContext::default();
    context.write_file(
        "snapshots/test__test_hello.snap",
        "---\nsource: not-a-source-reference\n---\nhello\n",
    );
    ...
    exit_code: 0
    ----- stdout -----
    Would remove: <temp_dir>/snapshots/test__test_hello.snap (invalid source metadata: not-a-source-reference)
