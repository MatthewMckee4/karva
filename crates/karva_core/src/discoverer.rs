        ...
        match path {
            PythonTestPath::File(path) => {
                discovered_tests.extend(
                    self.test_functions_in_file(path)
                        .into_iter()
                        .map(|function_name| DiscoveredTest::new(path.clone(), function_name)),
                );
            }
            PythonTestPath::Directory(path) => {
                for entry in std::fs::read_dir(path.as_std_path()).unwrap() {
                    let entry = entry.unwrap();
                    let path = SystemPathBuf::from(entry.path());

                    if path.is_dir() {
                        discovered_tests
                            .extend(self.discover_files(&PythonTestPath::Directory(path)));
                    } else if is_python_file(&path) {
                        discovered_tests.extend(
                            self.test_functions_in_file(&path)
                                .into_iter()
                                .map(|function_name| {
                                    DiscoveredTest::new(path.clone(), function_name)
                                }),
                        );
                    }
                }
            ...
            PythonTestPath::Function(path, function_name) => {
                ...
                if discovered_tests_for_file.contains(function_name) {
                    discovered_tests.push(DiscoveredTest::new(path.clone(), function_name.clone()));
                }
            }
        }
...
impl DiscoveredTest {
    pub fn new(path: SystemPathBuf, function_name: String) -> Self {
        Self {
            path,
            function_name,
        }
...
impl Display for DiscoveredTest {
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        write!(f, "{}::{}", self.path, self.function_name)
    }
}
