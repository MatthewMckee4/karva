...
impl Client {
    pub fn new(main_loop_sender: MainLoopSender, connection_sender: Sender<Message>) -> Self {
        Self {
            main_loop_sender,
            connection_sender,
        }
    ...
    pub fn send_request<R: Request>(&self, id: RequestId, params: R::Params) -> anyhow::Result<()> {
    ...
    pub fn publish_diagnostics(&self, publication: DiagnosticPublication) -> anyhow::Result<()> {
        self.main_loop_sender
    ...
    pub fn cancel(&self, session: &mut Session, id: RequestId) -> anyhow::Result<()> {
